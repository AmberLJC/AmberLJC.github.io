...
(function () {
  ...
  var timer = 0;
  var held = false;    // the reader is on the deck, so it waits for them
  var holds = { pointer: false, focus: false, hidden: false, offscreen: false };
  ...
  function measure() {
    ...
    var card = Math.round(Math.min(CARD_MAX, Math.max(230, w * 0.44)));
  ...
  function place() {
    ...
    dotList.forEach(function (dot, i) { dot.classList.toggle('is-active', i === active); });
    settled = Date.now();
  }
